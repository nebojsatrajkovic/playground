using Core.DB.Plugin.MySQL.Attributes;
using Core.DB.Plugin.MySQL.Utils;
...
namespace Core.DB.Plugin.MySQL.Database
{
    ...
    public class ADBTable<T1, T2> where T1 : new() where T2 : new()
    {
        ...
        public static List<T1> Search(CORE_DB_Connection dbConnection, T2 parameter)
        {
            ...
            try
            {
                var property_AlreadySaved = parameter?.GetType().GetProperties().FirstOrDefault(x => x.CustomAttributes.HasValue() && x.CustomAttributes.Any(a => a.AttributeType == typeof(CORE_DB_MySQL_AlreadySaved)));
        ...
        public static int SoftDelete(CORE_DB_Connection dbConnection, List<T1> parameter)
        {
            ...
            {
                var property = typeof(T1).GetProperties().FirstOrDefault(x => x.CustomAttributes.HasValue() && x.CustomAttributes.Any(a => a.AttributeType == typeof(CORE_DB_MySQL_PrimaryKey)));
        ...
        public static T1 Save(CORE_DB_Connection dbConnection, T1 parameter)
        {
            ...
            {
                var property = parameter?.GetType().GetProperties().FirstOrDefault(x => x.CustomAttributes.HasValue() && x.CustomAttributes.Any(a => a.AttributeType == typeof(CORE_DB_MySQL_AlreadySaved)));
    ...
    internal static class TypeExtensions
    {
        ...
        {
            var properties = t.GetProperties().Where(x => !x.CustomAttributes.HasValue() || !(x.CustomAttributes.HasValue() && x.CustomAttributes.Any(a => a.AttributeType == typeof(CORE_DB_MySQL_Ignore)))).ToList();
